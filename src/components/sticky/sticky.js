...
function anchorBottomCenter({x, y, height, width, offset}) {
  return {
...
type stickyProps = {
  children?: any,
  anchor?: 'top' | 'left' | 'bottom' | 'bottom center' | 'right' | 'none',
  transition?: string,
  width?: number,
  offset?: {
    x?: number,
...
export default class Sticky extends React.Component {
   ...
   */
  updatePosition() {
    const {anchor, offset, width, safeZone, zIndex, fixed} = this.props;
  ...
  renderChildren() {
    ...
    return <div {...omit(otherProps, ['offset'])}>{children}</div>;
  }
